            ...
            <div id="enable_quality_section" class="col-md-12 enable_quality_section d-none">
                <div id="video-inputs-container-parent">
                    <div class="row gy-3 video-inputs-container">
                        <!-- Hidden input for quality video type with HLS as default -->
                        ...
                        <div class="col-md-8 video-url-input quality_video_input">
                            {{ html()->label(__('movie.video_url_input') . ' (HLS)', 'quality_video_url_input')->class('form-label') }}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Validate HLS URL for main video
    function validateHlsUrlInput() {
        var videoUrlInput = document.querySelector('input[name="video_url_input"]');
        var urlPatternError = document.getElementById('url-pattern-error');
        
        // HLS URL pattern validation (can be .m3u8 or any valid URL)
        var hlsPattern = /^https?:\/\/.+$/;
        
        if (!hlsPattern.test(videoUrlInput.value)) {
            urlPatternError.style.display = 'block';
            return false;
        } else {
            urlPatternError.style.display = 'none';
...
// Add more video quality function
$('#add_more_video').on('click', function() {
    var newVideoInput = `
        <div class="row gy-3 video-inputs-container mt-3">
            <input type="hidden" name="video_quality_type[]" value="HLS" class="video_quality_type">
            <div class="col-md-4">
                <select name="video_quality[]" class="form-control select2 video_quality">
            ...
            <div class="col-md-8 video-url-input quality_video_input">
                <input type="text" name="quality_video_url_input[]" placeholder="Enter HLS URL" class="form-control">
            </div>
            <div class="col-sm-12 text-end mb-3">
    ...
    `;
    $('#video-inputs-container-parent').append(newVideoInput);
...
$(document).ready(function() {
    $('#import_movie').on('click', function(e) {
        ...
        $.ajax({
            url: '{{ route('backend.movies.import-movie', ':id') }}'.replace(':id', movieId),
            type: 'GET',
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    // Set HLS as default for video upload type
                    $('#video_upload_type').val('HLS');
                    ...
                    if (data.enable_quality === true) {
                        ...
                        data.entertainmentStreamContentMappings.forEach((video, index) => {
                            ...
                            videoInputContainer.innerHTML = `
                                ...
                                <div class="col-sm-8 mb-3 video-url-input quality_video_input">
                                    <label class="form-control-label" for="quality_video_url_input_${index}">Video URL (HLS)</label>
                                    <input type="text" name="quality_video_url_input[]" id="quality_video_url_input_${index}" placeholder="Enter HLS URL" class="form-control" value="${video.quality_video}">
                                </div>
            ...
            error: function(xhr) {
                ...
                if (xhr.responseJSON && xhr.responseJSON.status_message) {
                    $('#error_message').text(xhr.responseJSON.status_message || 'Failed to import movie details.');
                } else {
