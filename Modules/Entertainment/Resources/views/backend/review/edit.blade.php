...
@section('title')
    {{ __('messages.edit') }} {{ __($module_title) }}
@endsection
...
@section('content')
    <div class="card-main mb-5">
        <x-backend.section-header>
            <x-slot name="left">
                <h5 class="mb-0 text-white">{{ __('messages.edit') }} {{ __($module_title) }}</h5>
            </x-slot>
        </x-backend.section-header>

        <form action="{{ route('backend.reviews.update', $review->id) }}" method="POST" class="needs-validation" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="card-body form-dark">
                ...
                <div class="mb-3">
                    ...
                    <select name="entertainment_id" class="form-control select2 input-dark" required>
                        ...
                        @foreach ($entertainments as $entertainment)
                            <option value="{{ $entertainment->id }}" {{ $review->entertainment_id == $entertainment->id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="review" class="form-control input-dark" rows="4" placeholder="Enter Your Review" required>{{ $review->review }}</textarea>
                </div>
            </div>
